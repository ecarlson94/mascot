...
class ImagePickerWidget extends StatelessWidget {
  ...
  Future<ImageFile?> _onImageButtonPressed(
    ...
    try {
      Uint8List bytes = await _getImageBytes(source);

      if (bytes.isNotEmpty) {
        final imageFile = value.copyWith(bytes: bytes);
  ...
  Future<Uint8List> _getImageBytes(
    ...
    Uint8List? bytes;
    if (kIsWeb) {
      bytes = await _pickWebImage();
    } else {
      bytes = await _pickImage(source);
    }
    return bytes ?? Uint8List(0);
  }

  Future<Uint8List?> _pickWebImage() async {
